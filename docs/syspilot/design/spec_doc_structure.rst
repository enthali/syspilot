...
.. spec:: sphinx-needs Documentation Structure
   :id: SYSPILOT_SPEC_DOC_STRUCTURE
   :status: implemented
   :links: SYSPILOT_REQ_CORE_SPHINX_NEEDS, SYSPILOT_REQ_CORE_TRACEABILITY, SYSPILOT_REQ_CORE_DOMAIN_ORG, SYSPILOT_REQ_CORE_L1_MIRROR
   ...
   * User Stories: ``US_<THEME>_<SLUG>`` (e.g., ``SYSPILOT_US_CORE_SPEC_AS_CODE``)
   * Requirements: ``REQ_<THEME>_<SLUG>`` (e.g., ``SYSPILOT_REQ_CHG_ANALYSIS_AGENT``)
   * Designs: ``SPEC_<COMPONENT>_<SLUG>`` (e.g., ``SYSPILOT_SPEC_AGENT_WORKFLOW``)
...
.. needtable::
   ...
   :filter: id.startswith('SYSPILOT_SPEC_DOC')

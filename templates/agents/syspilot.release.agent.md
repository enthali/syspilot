...
- templates/version.json (expected - choose feature branch version)
...
```
Tasks:
- Read current version from templates/version.json
...
```markdown
Current version: 0.1.0 (from templates/version.json)
...
**Update version:**
```powershell
$version = Get-Content templates/version.json | ConvertFrom-Json
$version.version = "{NEW_VERSION}"
$version | ConvertTo-Json | Set-Content templates/version.json
```
...
git add docs/releasenotes.md templates/version.json
git commit -m "chore: release v{NEW_VERSION}

- Add release notes for v{NEW_VERSION}
- Update templates/version.json to {NEW_VERSION}
...
- Job 1: Validate templates/version.json matches tag
...
If `templates/version.json` doesn't match the requested version:

```
❌ ERROR: Version mismatch detected!

templates/version.json shows: 0.1.0
You requested: v0.2.0

Please update templates/version.json first.
```
...
❌ Validation failed:
  ...
  ✅ templates/version.json correct
...
Agent: Reading templates/version.json... Current version: 0.1.0
...
Agent: ✅ Approved. Proceeding with release...
       ...
       [Updates release notes, templates/version.json]
